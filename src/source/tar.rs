use super::DirEntry;
use crate::{utils::IdBuilder, SharedBytes, SharedString};
use std::{
    collections::HashMap,
    fmt, hash, io,
    path::{self, Path},
...
impl Tar<io::Cursor<SharedBytes>> {
    /// Creates a `Tar` archive backed by a byte buffer in memory.
    ///
    /// If you want to use another kind of byte buffer (such as `Arc<[u8]>`), you
    /// can use `from_reader`.
    #[inline]
    pub fn from_bytes(bytes: SharedBytes) -> io::Result<Self> {
        Tar::from_reader(io::Cursor::new(bytes))
    }
    ...
    pub fn from_bytes_with_label(bytes: SharedBytes, label: String) -> io::Result<Self> {
        Tar::from_reader_with_label(io::Cursor::new(bytes), label)
    }
}

impl<'a> Tar<io::Cursor<&'a [u8]>> {
    /// Creates a `Tar` archive backed by a byte buffer in memory.
    ///
